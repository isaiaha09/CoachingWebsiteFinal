...
def send_booking_mail(client_email, client_name, booking_details=None, custom_message=None):
    ...
    sender = SendSmtpEmailSender(email=settings.EMAIL_HOST_USER, name="Developmental Baseball")  # ✅ Correct object

    email = SendSmtpEmail(
        to=to_list,
        sender=sender,  # ✅ Correct format
        subject=booking_details.get("subject") if booking_details else "No Subject",
        html_content=custom_message or f"<p>Hello {client_name},<br>Your lesson has been booked!</p>",
    )

    try:
        response = api_instance.send_transac_email(email)
        logger.info(f"✅ Email sent successfully to {client_email}, response: {response}")
        return response
    except Exception as e:
